...
A comprehensive web-based platform for managing examinations, assessments, and academic activities in educational institutions. The platform supports three distinct user roles: Students, Lecturers, and Administrators, each with specific functionalities tailored to their needs.

## Project Overview

The EdTech Examination Management Platform is designed to streamline the examination process in educational institutions. It provides a robust system for creating, managing, and taking assessments with AI-powered grading capabilities.
...
#### Students
- View available, upcoming, and completed assessments
- Take theory-based assessments
- View AI-graded results
- Access personal dashboard and progress tracking
...
- **User Management**:
  ...
  - Add subsequent administrators with their employment ID
...
### Login System
- **Secure Authentication**: Users enter username and password only
- **Automatic Role Detection**: System determines user role from database credentials
...
- **Administrator Registration**: 
  - First admin created during system deployment using admin_registration_codes
  - Subsequent admins created directly by existing administrators
...
- Prevention of role-based login attempts (users cannot specify roles during login)
...
- `admin_registration_codes`: Secure initial admin creation codes (bootstrapping only)
...
4. Set up the first administrator account using admin_registration_codes
...
1. **System Setup**: Create initial administrator account using registration codes
...
### Lecturer Workflow
1. **Login**: Access lecturer dashboard (role automatically detected)
...
### Student Workflow
1. **Login**: Access student dashboard (role automatically detected)
...
- `POST /api/login`: User authentication (username/password only)
